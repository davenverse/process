...
trait ChildProcessCompanionPlatform {
  ...
  private[process] class ChildProcessImpl[F[_]: Async] extends ChildProcess[F]{
    def execSimple(proces: Process): F[(String, String)] = Async[F].async{(cb: Either[Throwable, (String, String)] => Unit) => 
      Async[F].delay{
        val commandS = proces.command ++ { if (proces.args.nonEmpty) " " else "" } ++ proces.args.mkString(" ")
    ...
    def execCode(proces: Process): F[Int] = Async[F].async{(cb: Either[Throwable, Int] => Unit) => 
      Async[F].delay{
        ...
        val process = internal.childProcessMod.spawn(proces.command, proces.args.toJSArray)
    ...
    def exec(process: Process): F[String] = execSimple(process).map(_._1)
